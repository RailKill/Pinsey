...
class Window(QtGui.QMainWindow):
    ...
    def populate_users(self, user_list):
        ...
        for user in user_list:
            ...
            # TODO: Add like, dislike and super like buttons.

            card = QtGui.QWidget()
            card.setLayout(QtGui.QGridLayout())
            card.layout().setSpacing(10)
            card.layout().addWidget(label_thumbnail, 1, 0, 7, 1)
            card.layout().addWidget(label_name, 1, 1)
            card.layout().addWidget(label_dob, 2, 1)
            card.layout().addWidget(label_distance, 3, 1)
            card.layout().addWidget(label_ping, 4, 1)
            card.layout().addWidget(label_schools, 5, 1)
            card.layout().addWidget(label_jobs, 6, 1)
            card.layout().addWidget(txt_bio, 7, 1)
